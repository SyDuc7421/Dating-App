...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ChatAdapter.MyViewHolder holder, int position) {
        ...
        if(list.getFromUID().equals(mAuth.getUid())){
            ...
            holder.txt_myMessage.setText(list.getMessage());
            holder.txt_myMessageTime.setText(String.format("%s %s", list.getDate(), list.getTime()));
        }
        else{
            holder.l_myLayout.setVisibility(View.GONE);
            holder.l_oppLayout.setVisibility(View.VISIBLE);
            holder.txt_oppMessage.setText(list.getMessage());
            holder.txt_oppMessageTime.setText(String.format("%s %s", list.getDate(), list.getTime()));
        }
    }
